...
class BAtten(nn.Module):
    def __init__(self, dim, template=13, heads=8, dropout=0., cls=False, window=5, pos_embbing=False, bias=False,
        ...
        self.cls = cls
        if self.cls:
            self.cls_tokens = nn.Parameter(torch.randn(dataset_num, 1, 1, dim))
        ...
        if pos_embbing and pos_embbding_mode == 'absolute':
            self.pos_emb_kvs = nn.Parameter(torch.zeros(dataset_num, 1, window * window * self.filt_num, dim))
            self.pos_emb_qs = nn.Parameter(torch.zeros(dataset_num, 1, window * window, dim))
        if pos_embbing and pos_embbding_mode == 'relative':
            self.relative_position_bias_tables = nn.Parameter(
                torch.zeros(dataset_num, (2 * window - 1) * (2 * window - 1), heads))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, dataset_id = 0):
        b, c, h, w = x.shape
        heads = self.heads
        ...
        if h_block == h and w_block == w:
            ...
            if self.pos_embbing and self.pos_embbding_mode == 'absolute':
                kv_0 = kv_0 + self.pos_emb_kvs[dataset_id]
                q_0 = q_0 + self.pos_emb_qs[dataset_id]
            if self.cls:
                cls = self.cls_tokens[dataset_id].repeat([int(h_block * w_block / self.window ** 2), 1, 1])
            ...
            if self.pos_embbing and self.pos_embbding_mode == 'relative':
                relative_position_bias = self.relative_position_bias_tables[dataset_id][self.relative_position_index.view(-1)].view(
            ...
            if self.pos_embbing and self.pos_embbding_mode == 'absolute':
                kv_0 = kv_0 + self.pos_emb_kvs[dataset_id]
                kv_1 = kv_1 + self.pos_emb_kvs[dataset_id]
                q_0 = q_0 + self.pos_emb_qs[dataset_id]
                q_1 = q_1 + self.pos_emb_qs[dataset_id]
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, template, mlp_mult, window_size, cls=False, dropout=0., bn=False,
                 ...
                 cnor_val=4, tempt=1, dataset_num = 1):
        super().__init__()
    ...
    def forward(self, x, dataset_id = 0):
        ...
        for ly1, attn, ly2, ff in self.layers:
            y, attenout = attn(ly1(x), dataset_id)
            x = y + x
            y = ff(ly2(x))
            x = y + x
            fea.append(x)
...
class TDCN(nn.Module):
    ...
    def forward(self, img, dataset_id = 0):
        ...
        for level, (conv, transformer, aggregate, aggregate_t) in zip(range(num_hierarchies), self.layers):
            x, fea_c = conv(x)
            if level == 0:
                _, fea_t, attenout = transformer(x, dataset_id)
            else:
                _, fea_t, attenout = transformer(x + aggregate_t(featurelist[level - 1][0]), dataset_id)
